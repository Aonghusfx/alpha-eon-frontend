...
export function PaymentWorkflow({
  ...
  // IMPORTANT: Only use locationId from externalParams - do NOT use environment variable
  const advitalLocationId = externalParams?.locationId || '';
  ...
  // Validate upfront payment values
  useEffect(() => {
  ...
  useEffect(() => {
    const handleAdvitalMessage = (event: MessageEvent) => {
      ...
      if (message.type === 'advital_payment_success') {
        ...
        console.log('📊 Current payment data before update:', currentPaymentData);
        console.log('📊 Selected plan:', currentPaymentData.selectedPlanObject);
              ...
              <SuccessStep
                ...
                onSignatureConfirmed={async () => {
                  ...
                  if (!externalParams?.orderId || !advitalLocationId) {
                    ...
                    toast.error('Cannot record payment: Missing invoice information');
                    return;
                  }

                  console.log("🚀 Calling Alphaeon callback API...");
                  console.log("  - Invoice ID (GHL):", externalParams.orderId);
                  console.log("  - Location ID (GHL):", advitalLocationId);
                  ...
                  await notifyAlphaeonFinancingComplete({
                    ...
                    amount: orderAmount,
                    transactionId: paymentData.transactionId ? String(paymentData.transactionId) : undefined
                  });
